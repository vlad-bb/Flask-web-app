...
def move_user_pic(user_id, file_path: Path):
    ...
    file = file_path.rename(
        target_folder / Path(str(datetime.now().strftime("%I_%M_%S")) + file_path.name))  # /src/static/101/name.jpg
...
def move_user_mp3(user_id, file_path: Path):
    target_folder = Path(BASE_DIR / "src" / "static" / str(user_id))  # /src/static/101/
    target_folder.mkdir(exist_ok=True)
    # path = file_path.rename(target_folder / Path(str(file_path.name))) # /src/static/101/name.jpg
